    ...
    execute: function () {
      ...
      _export("Player", Player = (_dec = ccclass('Player'), _dec2 = property({
        ...
        onTouchMove(event) {
          ...
          const targetPosition = new Vec3(position.x + event.getDeltaX(), position.y + event.getDeltaY(), position.z); // 判断边界
          // 左边界
        ...
        beginContact(selfCollider, otherCollider, contact) {
          // 判断是否撞到道具
          const tool = otherCollider.getComponent(_crd && Tool === void 0 ? (_reportPossibleCrUseOfTool({
            error: Error()
          }), Tool) : Tool);
        ...
        oneBullet(deltaTime) {
          ...
          if (this.bulletTimer >= this.bulletTime) {
            ...
            let bullet = instantiate(this.bulletPrefab1); // 添加到父节点
        ...
        twoBullet(deltaTime) {
          ...
          if (this.bulletTimer >= this.bulletTime) {
            ...
            let bullet2 = instantiate(this.bulletPrefab2); // 添加到父节点
...
//# sourceMappingURL=3d5c09b5fba4e027a8480c711f5803758294f1b4.js.map